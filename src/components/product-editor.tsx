    ...
    <Card>
      ...
      <form action={formAction} className="mt-4 grid min-w-0 gap-4">
        ...
        <div>
          ...
          <TextInput name="name_ar" required defaultValue={product?.name_ar} className="min-h-12" />
        </div>
        <div>
          <FieldLabel>الاسم الإنجليزي</FieldLabel>
          <TextInput name="name_en" defaultValue={product?.name_en ?? ""} className="min-h-12 ltr" />
        </div>
        <div>
          <FieldLabel required>التصنيف</FieldLabel>
          <select name="category_id" required defaultValue={product?.category_id} className="min-h-12 w-full min-w-0 rounded-2xl border border-[var(--border)] bg-white/80 px-4">
        ...
        <div>
          ...
          <TextInput name="price_iqd" type="number" min="0" step="1" defaultValue={product?.price_iqd ?? ""} className="min-h-12 ltr" placeholder="اختياري" />
        </div>
        <div>
          <FieldLabel>حالة المنتج</FieldLabel>
          <select name="active" defaultValue={product?.active === false ? "false" : "true"} className="min-h-12 w-full rounded-2xl border border-[var(--border)] bg-white/80 px-4">
        ...
        <div>
          ...
          <TextInput name="sort_order" type="number" defaultValue={product?.sort_order ?? 0} className="min-h-12 ltr" />
        </div>
        <div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <a href="/admin/products" className="inline-flex min-h-12 items-center rounded-2xl px-5 font-bold text-[var(--olive)]">
            إلغاء
          </a>
        </div>
      </form>
      {product ? (
        ...
        >
          ...
          <Button type="submit" variant="secondary" className="mt-3 min-h-11">
            رفع الصورة المرجعية
          </Button>
      ...
      <form action={createProductCategoryAction} className="mt-6 grid gap-2 rounded-2xl border border-dashed border-[var(--border)] p-4">
        ...
        <TextInput name="name_ar" placeholder="اسم التصنيف بالعربي" className="min-h-11" />
